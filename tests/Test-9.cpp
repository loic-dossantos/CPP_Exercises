...
public:
    std::unique_ptr<Monster> make_random_monster() {
        return std::make_unique<Monster>(make_random_id(), "Monster", make_random_attribute(), "Type",
                                         make_random_int(0, 3000), make_random_int(0, 3000));
    }

    std::unique_ptr<Spell> make_random_spell() {
        return std::make_unique<Spell>(make_random_id(), "Spell", make_random_spell_type());
    }

    std::unique_ptr<Trap> make_random_trap() {
        return std::make_unique<Trap>(make_random_id(), "Trap", make_random_trap_type());
    }
...
private:
    ...
    std::string make_random_id() { return "ID-FR" + std::to_string(make_random_int(1, 2000)); }

    Attribute make_random_attribute() { return static_cast<Attribute>(make_random_int(0, 6)); }

    SpellType make_random_spell_type() { return static_cast<SpellType>(make_random_int(0, 4)); }

    TrapType make_random_trap_type() { return static_cast<TrapType>(make_random_int(0, 2)); }
